# In[9]:
###########################################################################################################################
#Objective: To Predict which Passengers Survived the Titanic using Decision Trees, RF, and GBM with Best Practices        #
#Before you run the script please change your working directory to the "Full Python Pipeline Titanic" File Folder Location#
###########################################################################################################################
...
def calc_iv(var_name,continuous_target_nx,IV_lst):
...
def cont_bin(df,df_continuous,n_bin,target_col,filename):
    ...
    for i in range(len(df_continuous_column)):
        if (df_continuous_column[i] != target_col):  
            ...
            df_fnl=create_volume_group(df_continuous,df_continuous_column[i],target_col,n_bin)
            df_fnl=df_fnl[[df_continuous_column[i],target_col,'Decile']]

            if df_fnl.eval(df_continuous_column[i]).isnull().sum() > 0:
                continuous_target_nx=cont_bin_Miss(df_fnl,df_continuous_column,i,target_col,filename)                
            else:
                continuous_target_nx=cont_bin_NO_Miss(df_fnl,df_continuous_column,i,target_col,filename)
            
            data_fnl,IV_lst=calc_iv(df_continuous_column[i],continuous_target_nx,IV_lst)
...
def plot_stat(df_continuous,title_name,target_col,df_plot):
    ...
    plt.clf()    
    return df_plot,img_name,work_dir,scatter_img_name
...
def add_table_plot(df_continuous,in_file,sheet_nm,out_file,target_col):
    ...
    for i in range(len(df_cont)):
        ...
        if df_cont.iloc[:,0][i] == "Total":
            ...
            df_plot,img_name,work_dir,scatter_img_name = plot_stat(df_continuous,title_name,target_col,df_plot)            
...
def cont_bin(df,df_continuous,n_bin,target_col,filename):
    ...
    writer.save()
    return continuous_target_nx,df_fnl
    
continuous_target_nx,df_fnl = cont_bin(df,df_continuous,n_bin=10,target_col='Survived',filename='Continuous_base_feb24.xlsx')        
...
def calc_iv(var_name,categorical_target_nx,IV_lst):
...
def cat_bin(df,df_categorical,target_col,filename):
    ...
    for i in range(len(df_categorical_column)):
        ...
        if (df_categorical_column[i] != target_col):       
            ...
            categorical_target_nx=cat_bin_trend(df_cat_fnl,df_categorical_column,i,target_col,filename)                                   
            data_fnl,IV_lst=calc_iv(df_categorical_column[i],categorical_target_nx,IV_lst)
...
def plot_stat(df_categorical,title_name,target_col,df_plot):
...
def add_table_plot(df_categorical,in_file,sheet_nm,out_file,target_col):
    ...
    for i in range(len(df_cont)):
        ...
        if df_cont.iloc[:,0][i] == "Total":
            ...
            df_plot=df_plot.reset_index()
            df_plot,img_name,work_dir,scatter_img_name = plot_stat(df_categorical,title_name,target_col,df_plot)
...
categorical_target_nx= cat_bin(df,df_categorical,target_col='Survived',filename='Categorical_base_feb24.xlsx')       
...
decision_tree_model = decision_tree_classification(X_train,y_train,X_test,y_test)
...
#Maximum number of levels in the tree
max_depth = [int(x) for x in np.linspace(start = 8, stop = 9, num = 2)]
...
df_gs_result = format_grid_search_result(GS_rfc.cv_results_)
...
clf = random_forest_classification(X_train,y_train,X_test,y_test)
...
#Maximum number of levels in the tree
max_depth = [int(x) for x in np.linspace(start = 9, stop = 10, num = 2)]
...
df_gs_result = format_grid_search_result(GS_gbm.cv_results_)
df_gs_result.to_excel('Gradient_Boosting_Machine_Grid_Search_24feb.xlsx')


# # Gradient Boosting Machine : Optimized Model
def gradient_boost_classification(X_train,y_train,X_test,y_test):
...
gbm = gradient_boost_classification(X_train,y_train,X_test,y_test)
#Lift Gain Chart
select_var=['Pclass','Child_ind','Fare','Sex_WOE','Dependent_ind_WOE','Embarked_ind_WOE']
